...
"""
The module ``grid.py`` implements the :py:class:`Grid` class which encapsulates
the functionality of a Resource Grid including:

- Keeping the Resource Element (RE) values for a specified resource grid size.
- Providing easy access to a specific type of data in the resource grid (e.g.
  DMRS values, CSI-RS values, PDSCH data, etc.)
...
- Applying `OFDM <https://en.wikipedia.org/wiki/Orthogonal_frequency-division_multiplexing>`_
  modulation to the resource grid which results in a :py:class:`~neoradium.waveform.Waveform`
  object.
...
- Performing *Synchronization* based on the correlation between the configured
  :doc:`reference signals <./RefSig>` and a received :py:class:`~neoradium.waveform.Waveform`.
...
from .utils import polarInterpolate, interpolate, herm
...
class Grid:
    r"""
    This class implements the functionality of a resource grid. It keeps the
    complex frequency-domain values of resource elements (REs) in a resource
    grid.
    """
    # ************************************************************************************************************************************************
    # ReTypes:
    ...
    retColors = list(retColors) + (retMaxPredefine+retMaxCustom-len(retColors))*[retColors[0]]  # Fill unused colors with the color of "UNASSIGNED"
    ...
    def __init__(self, bwp, numPlanes=1, contents="DATA", useReDesc=False, numSlots=1):
        ...
        numPlanes : int (default: 1)
            A resource grid can be considered as a 3-dimensional ``P x L x K`` complex
            tensor where ``L`` is the number of OFDM symbols, ``K`` is the number of
            subcarriers (based on ``bwp``), and ``P`` is the number of *planes*. In different
            contexts, ``P`` can be equivalent to the number of layers, number of transmitter
            antenna, or number of receiver antenna. To avoid any confusion, the resource
            grid implementation in **NeoRadium** uses the term *"Plane"* for the first
            dimension of the resource gird.
        
        contents : str (default: "DATA")
            The default content type of this resource grid. Each resource element (RE)
            in the resource grid has a an associated content type. When some data is
            assigned to some REs in this resource grid without a specified content
            type, this default value is used. The following content types are currently
            defined:
            
            :DATA: A Generic content type used when the type of data in the resource
                grid is unknown or not specified.
            :PDSCH: The content type used for the data carried in a Physical Downlink
                Shared Channel (PDSCH)
            :PDCCH: The content type used for the data carried in a Physical Downlink
                Control Channel (PDCCH)
            :PUSCH: The content type used for the data carried in a Physical Uplink
                Shared Channel (PUSCH)
            :PUCCH: The content type used for the data carried in a Physical Uplink
                Control Channel (PUCCH)
            :RX_DATA: The content type used for the received resource grid. (Created
                by the OFDM demodulation process)
        ...
        numSlots : int (default: 1)
            The number of time slots to include in the resource grid. The number
            of time symbols ``L`` (the second dimension of the resource grid tensor)
            is equal to ``numSlots*bwp.symbolsPerSlot``.
        ...
        **Other Read-Only Properties:**
        ...
        Here is a list of additional properties:
            ...
            :numRBs: The number of resource blocks (RBs) in this resource grid. This
                is equal to ``numSubcarriers/12``. The number of subcarriers in a
                resource grid is always a multiple of 12.
                
            :numSymbols: The number of time symbols in this resource grid. This
                is equal to ``numSlots*bwp.symbolsPerSlot``.
                
            :size: The size of resource grid tensor.
            
            :noiseVar: The variance of the AWGN noise present in this resource grid.
                This is usually initialized to zero. When an AWGN noise is applied
                to the grid using the :py:meth:`addNoise` function, the variance of
                the noise stored in this property. Also, if a noisy
                :py:class:`~neoradium.waveform.Waveform` is OFDM-demodulated using the
                :py:meth:`~neoradium.waveform.Waveform.ofdmDemodulate`
                method, then the amount of noise is transferred to the new
                :py:class:`~neoradium.grid.Grid` object created.
            
        Additionally you can access (Read-Only) these :py:class:`~neoradium.carrier.BandwidthPart`
        class properties directly: ``startRb``, ``numRbs``, ``nFFT``, ``symbolsPerSlot``,
        ``slotsPerSubFrame``, ``slotsPerFrame``, ``symbolsPerSubFrame``.
        
        **Resource Grid Indexing:**
        
        a) *Reading*: You can directly access the contents of the resource grid using
        indexes. Here are a few examples of accessing the RE values in the resource
        grid:
        ...
        b) *Writing*: You can assign different values to different REs in the
        resource grid. Here are a few example:
        ...
        self.reDesc = None
        if useReDesc:   self.reDesc = np.array(np.prod(gridShape) * ["UNASSIGNED"], dtype=np.dtype('<U20')).reshape(gridShape)
        self.noiseVar = 0
    ...
    def getStats(self):
        r"""
        Returns some statistics about the allocation of resources in the
        resource grid.

        Returns
        -------
        dict
            A dictionary of items containing the number of resource elements
            allocated for different types of data in this resource grid.
        """
    ...
    def __getattr__(self, property):        # Not documented (Already mentioned in the __init__ documentation)
        ...
        if property not in ["startRb", "numRbs", "nFFT", "symbolsPerSlot", "slotsPerSubFrame", "slotsPerFrame", "symbolsPerSubFrame"]:
    ...
    # ************************************************************************************************************************************************
    def __getitem__(self, key):             # Not documented (Already mentioned in the __init__ documentation section Resource Grid Indexing)
    ...
    # ************************************************************************************************************************************************
    def __setitem__(self, key, values):     # Not documented (Already mentioned in the __init__ documentation section Resource Grid Indexing)
        ...
        if type(values) == tuple:                           # For example: grid[1,2,3] = (123, "DMRS")      -> Assigning DMRS values
            ...
            values, retId = values, self.defaultReType      # For example: grid[1,2,3] = 123                -> Assigning data (such as "PDSCH")
    ...
    def reTypeAt(self, p, l, k):
        r"""
        Returns the content type (as a string) of the resource element at the
        position specified by ``p``, ``l``, and ``k``.

        Parameters
        ----------
        p: int
            The *plane* number. It can be the layer or antenna index depending
            on the context.
        ...
        -------
        str
            The content type of the resource element specified by ``p``,
            ``l``, and ``k``.
        """
    ...
    def getReIndexes(self, reTypeStr=None):
        r"""
        Returns the indexes of all resource elements in the resource grid with
        the content type specified by the ``reTypeStr``. For example, the following
        code first gets the indexes of all DMRS values in the resource grid and uses
        the returned indexes to retrieve the DMRS values.
        ...
        reTypeStr: str or None (default: None)
            If ``reTypeStr`` is ``None``, the default content type of this resource
            grid is used as the key. For example if this resource grid was created
            with ``contents="PDSCH"``, then the indexes of all resource elements
            with content type "PDSCH" are returned.
            
            Otherwise, this function returns the indexes of all resource elements
            in the resource grid with the content type specified by ``reTypeStr``.
            Here is a list of values that can be used:
                ...
                :"NO_DATA": The resource elements that should not contain any data.
                    ...
                    used for transmission of data for a differnt UE. (See ``otherCdmGroups``
                ...
                :"PUSCH": The resource elements used for user data in a Physical Uplink
                    Shared Channel (:py:class:`~neoradium.pdcch.PUSCH`)
                :"PUCCH": The resource elements used for user data in a Physical Uplink
                    Control Channel (:py:class:`~neoradium.pdcch.PUCCH`)

                    
        Returns
        -------
        3-tuple
            A tuple of three 1-D numpy arrays specifying a list of locations in the
            resource grid. This value can be used directly to access REs at
            the specified locations. (See the above example)
        """
    ...
    def getReValues(self, reTypeStr=None):
        r"""
        Returns the values of all resource elements in the resource grid with
        the content type specified by the ``reTypeStr``. This is a short cut
        method that allows accessing all the values in one step. For example,
        ...
        reTypeStr: str or None (default: None)
            If ``reTypeStr`` is ``None``, the default content type of this resource
            grid is used as the key. For example, if this resource grid was created
            with ``contents="PDSCH"``, then the values of all resource elements
            with content type "PDSCH" are returned.
            
            Otherwise, this function returns the values of all resource elements
            in the resource grid with the content type specified by ``reTypeStr``.
            See :py:meth:`getReIndexes` for a list of values that could be used
            for ``reTypeStr``.
                    
        Returns
        -------
        1-D numpy array
            A 1-D complex numpy array containing the values for all REs with
            the content type specified by ``reTypeStr``.
        """
    ...
    def precode(self, f):
        r"""
        Applies the specified precoding matrix to this grid object and returns
        a new *precoded* grid. This function supports *Precoding RB groups (PRGs)*
        which means different precoding matrixes could be applied to different groups
        of subcarriers in the resource grid. See **3GPP TS 38.214, Section 5.1.2.3**
        for more details.
        ...
        f: numpy array or list of tuples
            ...
            :Wideband: ``f`` is an ``Nt x Nl``, matrix where ``Nt`` is the number
                of transmitter antenna and ``Nl`` is the number of layers which
                **must** match the number of layers in the resource grid. In this
            ...
            :Using PRGs: ``f`` is a list of tuples of the form (``groupRBs``, ``groupF``).
                For each entry in the list, the ``Nt x Nl`` precoding matrix ``groupF`` is
                applied to all subcarriers of the resource blocks listed in ``groupRBs``.

        Returns
        -------
        :py:class:`~neoradium.grid.Grid`
            A new Grid object of shape ``Nt x L x K`` where ``Nt`` is the number of
            transmitter antenna, ``L`` is the number of OFDM symbols, and ``K`` is the
        ...
        if type(f)==list:
            ...
            if f.shape[1] != self.numLayers:
                raise ValueError("The last dimension of 'f' (%d) must match the first dimension of the grid (%d)"%(f.shape[-1],self.shape[0]))
            #      f   . self.grid      ->      precodedGrid        <--- Tensors
    ...
    def ofdmModulate(self, f0=0, windowing="STD"):
        ...
        :py:class:`~neoradium.waveform.Waveform` object. This function is based
        on **3GPP TS 38.211, Section 5.3.1**.

        Parameters
        ----------
        f0: float (default: 0)
            The carrier frequency of the generated waveform. If it is 0 (default),
            then a baseband waveform is generated and the *up-conversion* process
            explained in **3GPP TS 38.211, Section 5.4** is not applied.

        windowing: str (default: "STD")
            A text string indicating what type of windowing should be applied to
            the waveform after OFDM modulation. The default value ``"STD"`` means
            the windowing should be applied based on **3GPP TS 38.104, Sections B.5.2
            and C.5.2**. For more information see
            :py:meth:`~neoradium.waveform.Waveform.applyWindowing` method of the
            :py:class:`~neoradium.waveform.Waveform` class.
            
        Returns
        -------
        :py:class:`~neoradium.waveform.Waveform`
            A :py:class:`~neoradium.waveform.Waveform` object containing the OFDM-modulated
        ...
        numPad = ((self.nFFT-kk+1)//2,(self.nFFT-kk)//2)                # Number of zeros to pad to the beginning and end of subcarriers
        ...
        indexes = (np.arange(maxSymLen) - cpLens[:,None])%self.nFFT     # Indexes used to insert the CP-Len elements from the end of symbol waveforms to the beginning.

        waveformWithCPs = np.zeros((pp,ll, maxSymLen), dtype=np.complex128)     # Shape: pp,ll, maxSymLen
        for l in range(ll): waveformWithCPs[:,l,:] = waveform[:,l,indexes[l]]   # Insert the CP-Len elements from the end of symbol waveforms to the beginning
        ...
        waveform = Waveform(np.concatenate([waveformWithCPs[:,l,:symLen] for l,symLen in enumerate(symLens)], axis=1)) # Shape: pp, waveformLen
    ...
    def estimateTimingOffset(self, rxWaveform):
        r"""
        Estimates the timing offset of a received waveform. This method first
        applies OFDM modulation to the resource grid and then calculates the
        correlation of this waveform with the given ``rxWaveform``. The timing
        offset is the index of where the correlation is at its maximum. The
        output of this function can be used by the
        :py:meth:`~neoradium.waveform.Waveform.sync` method of the
        :py:class:`~neoradium.waveform.Waveform` class to synchronize a
        received waveform.

        Parameters
        ----------
        rxWaveform: :py:class:`~neoradium.waveform.Waveform`
            The :py:class:`~neoradium.waveform.Waveform` object containing the received
            waveform.
            
        Returns
        -------
        int
            The timing offset in number of time-domain samples. This is the
            number of samples that should be ignored from the beginning of the
            ``rxWaveform``.
        """
    ...
    def equalize(self, hf, noiseVar=None):
        r"""
        Equalizes a received resource grid using the estimated channel ``hf``. The
        the estimated channel is assumed to include the effect of precoding
        matrix, therefore, its shape is ``L x K x Nr x Nl`` where ``L`` is the
        number of OFDM symbols, ``K`` is the number of subcarriers, ``Nr`` is
        the number of receiver antenna, and ``Nl`` is the number of layers. The
        output of the equalization process is a new :py:class:`Grid` object
        of shape ``Nl x L x K``.
        
        This function also outputs Log-Likelihood Ratios (LLR) scaling factors
        which are used by the demodulation process when extracting Log-Likelihood
        Ratios (LLRs) from the equalized resource grid.
        
        This method uses the Minimum Mean Squared Error (MMSE) algorithm for the
        equalization.

        Parameters
        ----------
        hf: 4-D complex numpy array
            This is an ``L x K x Nr x Nl`` numpy array representing the estimated
            channel matrix, where ``L`` is the number of OFDM symbols, ``K`` is
            the number of subcarriers, ``Nr`` is the number of receiver antenna,
            and ``Nl`` is the number of layers.
            
        noiseVar: float or None (default: None)
            The variance of the noise applied to the received resource grid. If
            this is not provided, this method tries to use the noise variance
            of the resource grid obtained by the OFDM demodulation process for
            the time-domain case or the variance of the noise applied to the
            received resource grid by the :py:meth:`addNoise` method for the
            frequency domain case (See the ``noiseVar`` property of
            :py:class:`Grid` class).
            
        Returns
        -------
        eqGrid: :py:class:`Grid`
            The equalized grid object of shape ``Nl x L x K`` where ``Nl``
            is the number of layers, ``L`` is the number of OFDM
            symbols, and ``K`` is the number of subcarriers.
        ...
        llrScales: 3-D numpy array
            ...
            from the equalized resource grid. The shape of this array is
            ``Nl x L x K`` which is similar to ``eqGrid`` above.
        """
        ...
        if (self.shape[0] != hf.shape[2]) or (self.shape[1] != hf.shape[0]) or (self.shape[2] != hf.shape[1]):
            raise ValueError("Mismatch in the number of receiver antenna, OFDM symbols, or subcarriers!")
    ...
    def scaleNoiseVar(self, rawNoiseVar, numTx, lCdm, kCdm, numVar):    # Not documented
        ...
        # noise variance.
        rr, _, kk = self.shape             # Number of RX antenna, Number of subcarriers

        rawSnrDb = 10*np.log10( 1/(rawNoiseVar * rr) )
        ...
        #   3) Number of layers (or Tx antenna)
        #   4) Number of RX antenna
    ...
    def estimateChannelLsEx(self, rsInfo, meanCdm=True, polarInt=True, int2d=True,
        ...
        if rsInfo.__class__.__name__ == "CsiRsConfig":
            ...
            # For the case of DMRS, the returned channel (Heff) includes the effect of precoding. If 'V' is the precoding
            # matrix, we have y = H.V.x + n. This function returns Heff = H.V.
            dmrs = rsInfo
        ...
        cdmSize = lCdm * kCdm
        rr, ll, kk = self.shape             # Number of RX antenna, Number of symbols, Number of subcarriers
        pp, ll2, kk2 = rsGrid.shape         # Number of Ports/Layers, Number of symbols, Number of subcarriers (From Reference Signals)
        if (ll!=ll2) or (kk!=kk2): raise ValueError("The Gird size (%dx%d) does not match Reference Signals (%dx%d)."%(ll,kk,ll2,kk2))
        
        hEstAtPilots = []       # Channel Estimates at pilot locations. A list of 'numLs x numKs x rr' tensors one for each port
        hEstAtPilotSyms = []    # Channel Estimates at pilot symbols interpolated along the subcarriers. A list of 'numLs x kk x rr' tensors one for each port
        for p in range(pp):
            ...
            rxValues = self.grid[:,ls,:][:,:,ks]                            # Received values for pilot signals in this port. Shape: rr x numLs x numKs
            hEst = np.transpose(rxValues/pilotValues[None,:,:], (1,2,0))    # Channel estimates at pilot locations transposed to Shape: numLs x numKs x rr
            ...
            if meanCdm:                                                     # Do CDM averaging
                ...
                if (numKs%kCdm>0) or (numLs%lCdm>0):    raise ValueError("Partial CDMs are not supported in this version.")
                
                # Calculate the mean of all CDM groups.
                hEst = np.transpose(hEst.reshape(numLs,-1,kCdm,rr), (0,2,1,3)).reshape(numLs//lCdm, cdmSize, -1, rr).mean(1) # Shape: numLs/lCdm x numKs/kCdm x rr

                if kCdm>1:  # Set the k values to the average subcarrier indexes in the CDM group
            ...
            if polarInt:    newVals = polarInterpolate(ks, vs, np.arange(kk), kernel, neighbors, smoothing)     # Shape: kk x numLs/lCdm x rr
            else:           newVals = interpolate(ks, vs, np.arange(kk), kernel, neighbors, smoothing)          # Shape: kk x numLs/lCdm x rr
            hEstInt = np.transpose(newVals,(1,0,2))                                                             # Shape: numLs/lCdm x kk, rr
        ...
        win = np.concatenate([ raisedCosine[::-1], np.float64((kk-2*riseLen)*[0]), raisedCosine])               # A window of shape: \__/ of len kk
        hEstDeltas = []         # A list of difference vectors between original pilot estimates and the denoised values for each port
        for p in range(pp):
            portLs = rsIndexes[1][(rsIndexes[0]==p)]            # Indexes of symbols containing pilots in this port
            ls = np.unique(portLs)                              # Unique Indexes of symbols containing pilots in this port
            ks = portKs[portLs==ls[0]]                          # Unique Indexes of subcarriers containing pilots in this port
            estCirs = np.fft.ifft( hEstAtPilotSyms[p], axis=1)  # Channel Impulse Responses (CIR).                        Shape: numLs/lCdm x kk x rr
            estCirsWin = estCirs * win[None,:,None]             # The CIR after applying the window.                      Shape: numLs/lCdm x kk x rr
            hEstDenoised = np.fft.fft(estCirsWin, axis=1)       # Back to frequency domain (Denoised estimated channel)   Shape: numLs/lCdm x kk x rr
        ...
        for p in range(pp):
            ...
            if meanCdm:     # Set the 'l' values to the average symbol indexes in the CDM group
            ...
            if int2d:
                ...
                f = RBFInterpolator(pilotLKs, pilotValues, neighbors, smoothing, kernel, degree=degree) # Setup the interpolant
    ...
    def estimateChannelLS(self, rsInfo, meanCdm=True, polarInt=False, kernel='linear'):
        r"""
        Performs channel estimation based on this received grid and the reference
        signal information in the ``rsInfo``. Here is a list of steps taken by this
        function to calculated the estimated channel and noise variance:
        
        1) First the channel information is calculated at each pilot location using least
        squared method based on the following equations:
        
        .. math::

            Y_p = h_p \odot P + n_p

        where :math:`Y_p` is a vector of received values at the pilot locations which are
        the values in this :py:class:`Grid` object at the pilot locations indicated in
        ``rsInfo``, :math:`h_p` is the vector of channel values at pilot locations, :math:`P`
        is the vector of pilot values extracted from ``rsInfo``, and :math:`n_p` is the
        noise at pilot locations. The least square estimate of the channel values at pilot
        ...
        2) If ``meanCdm`` is ``True``, the :math:`h_p` values in each CDM group are averaged
        which results in a new smaller set of :math:`h_p` values located at centers of
        CDM groups.
        
        3) Frequency interpolation along subcarriers is applied to :math:`h_p` values at all OFDM
        symbols containing pilots based on ``polarInt`` and ``kernel`` values.
        
        4) A *raise-cosine* low-pass filter is applied to the Channel Impulse Response
        (CIR) values to get a *de-noised* version of CIRs. The noise variance is estimated
        using the difference between the noisy and de-noised versions of the CIRs.
        
        5) Finally another interpolation is applied along OFDM symbols to estimate the
        channel information for the whole channel matrix.

        Parameters
        ----------
        rsInfo: :py:class:`~neoradium.csirs.CsiRsConfig` or :py:class:`~neoradium.pdsch.DMRS`
            This object contain reference signal information for the channel estimation. If
            it is a :py:class:`~neoradium.csirs.CsiRsConfig` object, the channel matrix is
            estimated based on the CSI-RS signals which does not include the precoding
            effect.
            
            If this is a :py:class:`~neoradium.pdsch.DMRS` object, the channel matrix is
            estimated based on the demodulation reference signals which includes the precoding
            effect.
            
        meanCdm: Boolean (default: True)
            If ``True``, the :math:`h_p` values at pilot locations for each CDM group
            are averaged before applying subcarrier interpolation. Otherwise interpolation
            is applied directly on the :math:`h_p` values.
            
        polarInt: Boolean (default: False)
            If ``True``, the interpolation along the subcarriers is applied in polar coordinates.
            This means all :math:`h_p` values are converted to the polar coordinates and
            then the type of interpolation specified by ``kernel`` is applied to magnitudes and
            angles of these values. The results are then converted back to the cartesian
            coordinates. Otherwise (default), the interpolation is applied in the cartesian
            coordinates.
            
            Doing polar interpolation provides slightly better results at the cost of longer
            execution time.
            
        kernel: str (default: 'linear')
            The type of interpolation used for channel estimation process. The same type of
            1-D interpolations are applied along subcarriers and then OFDM symbols. Here is a
            list of supported values:
            
            :linear: A linear interpolation is applied to the values using extrapolation at both
                ends of the arrays. This uses the function
                `interp1d <https://docs.scipy.org/doc/scipy/reference/generated/scipy.interpolate.interp1d.html>`_
                with ``kind`` set to ``linear``.

            :nearest: A nearest neighbor interpolation is applied to the values using extrapolation
                at both ends of the arrays. This uses the function
                `interp1d <https://docs.scipy.org/doc/scipy/reference/generated/scipy.interpolate.interp1d.html>`_
                with ``kind`` set to ``nearest``.

            :quadratic: A quadratic interpolation is applied to the values using extrapolation
                at both ends of the arrays. This uses the function
                `interp1d <https://docs.scipy.org/doc/scipy/reference/generated/scipy.interpolate.interp1d.html>`_
        ...
        hEst: a 4-D complex numpy array
            If ``rsInfo`` is a :py:class:`~neoradium.csirs.CsiRsConfig` object, an
            ``L x K x Nr x Nt`` complex numpy array is returned where ``L`` is the
            number of OFDM symbols, ``K`` is the number of subcarriers, ``Nr`` is
            the number of receiver antenna, and ``Nt`` is the number of transmitter
            antenna.
            
            If ``rsInfo`` is a :py:class:`~neoradium.pdsch.DMRS` object, an
            ``L x K x Nr x Nl`` complex numpy array is returned where ``L`` is the
            number of OFDM symbols, ``K`` is the number of subcarriers, ``Nr`` is
            the number of receiver antenna, and ``Nl`` is the number of layers.
    ...
    def applyChannel(self, channelMatrix):
        r"""
        Applies a channel to this grid in frequency domain which results in a
        new *received* :py:class:`Grid` object. This function performs a matrix
        multiplication where this grid of shape ``Nt x L x K`` is multiplied by the
        channel matrix of shape ``L x K x Nr x Nt`` and results in the *received* grid
        of shape ``Nr x L x K``, where ``L`` is the number of OFDM symbols, ``K`` is
        the number of subcarriers, ``Nr`` is the number of receiver antenna, and
        ``Nt`` is the number of transmitter antenna.
        
        This method can be used as a shortcut method to get the received resource grid
        faster compared to time domain process of doing OFDM modulation, applying the
        channel, performing synchronization, and doing OFDM demodulation.
        
        Please note that the results are slightly different when a channel is applied
        in time domain vs frequency domain.
        
        Parameters
        ----------
        channelMatrix: 4-D complex numpy array
            This is an ``L x K x Nr x Nt`` numpy array representing the estimated
            channel matrix, where ``L`` is the number of OFDM symbols, ``K`` is
            the number of subcarriers, ``Nr`` is the number of receiver antenna,
        ...
        -------
        :py:class:`Grid`
            The received grid of shape ``Nr x L x K``, where ``Nr``
            is the number of receiver antenna, ``L`` is the number of OFDM
            symbols, and ``K`` is the number of subcarriers.
        """
        ll, kk, nr, nt = channelMatrix.shape
        if nt != self.numPorts:
            raise ValueError("The number of transmitter antenna should be the same but they are %d and %d!"%(nt, self.numPorts))
    ...
    def addNoise(self, **kwargs):
        r"""
        Adds Additive White Gaussian Noise (AWGN) to this resource grid based on
        the given noise properties. The *noisy* grid is then returned in a new
        :py:class:`Grid` object. The ``noiseVar`` property of the returned grid
        ...
        kwargs: dict
            ...
            :noise: A numpy array with the same shape as this :py:class:`Grid`
                object containing the noise information. If the noise information
                is provided by ``noise`` it is added directly to the grid. In
            ...
            :noiseStd: The standard deviation of the noise. An AWGN complex noise
                ...
                deviation. If ``noiseStd`` is specified, ``noiseVar`` and ``snrDb``
            ...
            :noiseVar: The variance of the noise. An AWGN complex noise
                ...
                ``noiseVar`` is specified, the value of ``snrDb`` is ignored.

            :snrDb: The signal to noise ratio in dB. First the noise variance
                is calculated using the given SNR value and then an AWGN complex
                noise signal is generated with zero-mean and the calculated variance.
                This function uses the following formula to calculate the noise
                variance :math:`\sigma^2_{AWGN}` from :math:`snrDb`:
        ...
        :py:class:`Grid`
            ...
            ``noiseVar`` property of the returned grid contains the variance
            of the noise applied by this function.
        """
        noise = kwargs.get('noise', None)
        if noise is not None:
            if self.shape != noise.shape:
                raise ValueError("The resource grid shape %s does not match the noise shape %s!"%(str(self.shape), str(noise.shape)))
        ...
        if snrDb is not None:
            ...
            snr = 10**(snrDb/10)
            noiseVar = 1/(snr * self.shape[0])  # Note: self.shape[0] is the number of RX antenna
            return self.addNoise(noiseStd=np.sqrt(noiseVar), ranGen=ranGen)
    ...
    def drawMap(self, ports=[0], reRange=(0,12), title=None):
        r"""
        Draws a color-coded map of this grid object. Each ``plain`` of the grid is
        drawn separately with subcarriers in horizontal direction and OFDM symbols
        in vertical direction.
        
        Parameters
        ----------
        ports: list (default: [0])
            Specifies the list of ports (or ``plains``) to draw. Each port is drawn
            separately. By default this function draws only the first plain of the
            resource grid.
            
        reRange: tuple (default: (0,12))
            Specifies the range of subcarriers (REs) to draw. By default this function
            only draws the first resource block of the grid (subcarriers 0 to 12). The
            tuple of ``(a, b)`` means the first RE drawn is the one at ``a`` and last one
            is at ``b-1``.
           
        title: str or None (default: None)
            If specified, it is used as the title for the drawn grid. Otherwise, this
            function automatically creates a title based on the given parameters.
        """
        ...
        for p in ports:
            ...
            if p == ports[-1]:  # Draw the legend only for the last port
                ...
                plt.legend([patches.Patch(facecolor=self.retColors[dataType], edgecolor='black') for dataType in usedDataTypes],
